      ...
      if (p3) {
        if (/\d+/.test(p3)) {
          ...
          if (hours.length == 1) {
            ...
            hours =
              zh[hours[0]] + zh[hours[1]] + (zh[hours[2]] ? zh[hours[2]] : "");
          }
        }

        if (/中午/.test(p1 + p2))
          datetime +=
            1000 * 60 * 60 * (hours < 3 ? parseInt(hours) + 12 : hours);
        else if (/下午/.test(p1 + p2))
          datetime +=
            1000 * 60 * 60 * (hours <= 12 ? parseInt(hours) + 12 : hours);
...
/**
 * 格式化时间
 * @param time 放入new Date()
 * @param format 时间格式。默认"YYYY-MM-DD HH:mm:ss"
 * @returns format格式的时间字符串
 */
  ...
  if (/(y+)|(Y+)/.test(format))
    format = format.replace(
      ...
      (date.getFullYear() + "").substr(
        4 - (RegExp.$1.length || RegExp.$2.length)
      )
    );
  for (const k in o)
